...
/**
 * 当 cwd 包含必须受项目信任机制管控的项目本地资源时返回 true：
 * cwd/.pi 下需要信任的条目，或 cwd 及其任一祖先目录中的 .agents/skills。
 * 不存在此类项目资源时返回 false。用户/全局 ~/.agents/skills 目录始终
 * 视为可信的用户资源，因此即使 cwd 是 $HOME，此处也会忽略该目录。
 */
